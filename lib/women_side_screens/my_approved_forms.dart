...
class MyApprovedForms extends StatelessWidget {
  String type;

  MyApprovedForms({Key? key, required this.type});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      child: StreamBuilder<QuerySnapshot>(
          stream: type == "hall"
              ? FirebaseFirestore.instance.collection('woman_form').where("form_status", isEqualTo: "approved") .where("formApplierId", isEqualTo: FirebaseAuth.instance.currentUser!.uid).snapshots()
              : FirebaseFirestore.instance.collection('ServicesBooking').where("form_status", isEqualTo: "approved").where("formApplierId", isEqualTo: FirebaseAuth.instance.currentUser!.uid).snapshots(),
